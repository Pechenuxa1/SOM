...
export async function handleCheckboxChangeForFile(e, surveyId) {
    const isChecked = e.target.checked;

    try {
        loadingIndicator.classList.remove('hidden');
        
        const response = await fetch(`/api/files/${surveyId}?sort=${isChecked}`, {
            method: 'GET',
            headers: {
...
export async function addSurveyButtonHandler(button, surveyId) {
    ...
    button.addEventListener('click', async function() {
        ...
        // }
        checkbox.onchange = (e) => handleCheckboxChangeForFile(e, surveyId)
        //checkbox.addEventListener('change', (e) => handleCheckboxChangeForFile(e, surveyId));
        try {
            loadingIndicator.classList.remove('hidden');
